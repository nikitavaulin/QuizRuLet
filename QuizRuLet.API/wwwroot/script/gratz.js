document.addEventListener('DOMContentLoaded', async function () {
    ...
    const gratzPartially = `<div class="container d-flex align-items-center min-vh-100 flex-column">
        ...
        <a class="close-btn p-3 " id="close-btn" href="/">
        ...
        <div class="window p-4">
            ...
            <div class="modal-body align-items-center justify-content-center d-flex p-2 mb-4">
                <button class="btn-learn-notlearned" id="not-learned">Подучить ${countNotLearned} карточек</button>
                <button class="btn-learn-all" id="all">Пройти модуль заново</button>
            </div>
    ...
    try {
        document.getElementById('not-learned').addEventListener('click', async function (e) {
            e.preventDefault();
    ...
    try {
        document.getElementById('all').addEventListener('click', async function (e) {
            e.preventDefault();
            console.log(123);
            await axios.patch(`/cards/reset/${moduleId}`);
            window.location.href = `/card.html?id=${encodeURIComponent(moduleId)}`;
        })
    }
    catch { }
